...
1. DB에서 현재 상태 파악:
   ...
   SELECT * FROM mistake_patterns WHERE status = 'active' ORDER BY occurrence_count DESC;
   SELECT * FROM session_summaries ORDER BY session_id DESC LIMIT 1;
   ```
...
4. 핵심 파일 재실행하여 문서/DB 주장과 실제 상태 일치 확인:
   ```
   python3 spw_ref_model_test_v5.py 2>&1 | tail -20
   ```
